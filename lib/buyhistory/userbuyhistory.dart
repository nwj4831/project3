...
class _UserBuyHistoryState extends State<Userbuyhistory> {
  ...
  Future<void> _fetchOrderHistory() async {
    try {
      final response =
          await supabase.from('buyhistory').select().order('buy_id');
      setState(() {
  ...
  Future<Map<String, dynamic>?> _fetchData(
      String table, int id, List<String> columns) async {
  ...
  Widget _buildPaymentList(List<dynamic> payments, String table,
      String idColumn, List<String> columns) {
  ...
  Widget _buildListTile(
    ...
    return Container(
      decoration: const BoxDecoration(
        border: Border(
            bottom: BorderSide(color: Colors.grey, width: 0.5)), // 하단 border
      ),
      child: ListTile(
        title: Row(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Flexible(
                  child:
                      Text('가격: $price원', style: const TextStyle(fontSize: 20)),
                ),
                Flexible(
            ...
            Row(
              ...
              children: [
                Flexible(
                  child: Text('전화번호: $buyUphone',
                      style: const TextStyle(fontSize: 16)),
                ),
                Flexible(
                  child: Text('갯수: $buyEa',
                      style: const TextStyle(fontSize: 16)), // 갯수 출력
                ),
              ],
            ),
            const SizedBox(height: 8.0),
            ...
            Row(
              ...
              children: [
                Flexible(
                  child: Text('주문 날짜: $buyDate',
                      style: const TextStyle(fontSize: 16)),
                ),
                Flexible(
                  child: Text('총 가격: $totalPrice원',
                      style: const TextStyle(fontSize: 16)), // 총 가격 출력
                ),
              ],
            ),
            const SizedBox(height: 8.0),
          ...
          Navigator.push(
            ...
            MaterialPageRoute(
              builder: (context) => DetailBuyHistory(
                  data: {'title': title, 'price': price, ...order}),
            ),
          );
        },
      ),
